...
def test_structured_indicators_generation():
    ...
    try:
        ...
        _context = {"recentPivotBarsAgo": 2, "priceBand": "mid", "entry_price": 7520, "position_type": "long"}

        _image_base64 = create_test_image()
        ...
        mock_pivot_result = {
            ...
            "final": 80.75,
            "isPivot": True,
            "meta": {"near20": True, "near60": False, "slope20pct": 2.1, "slope60pct": 1.7, "priceBand": "mid"},
        }
        ...
        mock_gpt_result = {
            ...
            "macd": {
                "value": "ゴールデンクロス",
                "evaluation": "強気",
                "comment": "MACDラインがシグナルを上抜け、ヒストグラム拡大",
            },
            "bollinger_band": {
                "value": "バンド中央上",
                "evaluation": "やや強気",
                "comment": "ボリンジャーバンド中央線上で上昇余地あり",
            },
            "trend": {
                "value": "上昇トレンド",
                "evaluation": "強気",
                "comment": "高値・安値を切り上げ、明確な上昇トレンド",
            },
...
def main():
    ...
    if all_passed:
        ...
        print("✅ structured_indicators配列が正しく生成されています")
        print("✅ Jinja2テンプレートで自然文フィードバックが生成されています")
        print("✅ ルールベースとGPT解析の役割が適切に分離されています")
    else:
