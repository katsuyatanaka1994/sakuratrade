...
async def generate_ai_reply(
    ...
    try:
        ...
        if not chat:
            raise HTTPException(
                status_code=404,
                detail=f"Chat with ID {request.chatId} not found"
            )
        ...
        if not user_message:
            raise HTTPException(
                status_code=404,
                detail=f"User message with ID {request.latestUserMessageId} not found"
            )
        
        # TODO: 実際のAI生成ロジックを実装
        # 現在はモック応答を生成
        ai_response = f"AI応答が生成されました。メッセージの更新/Undo後の再分析結果です。（参照メッセージID: {request.latestUserMessageId}）"
        ...
        # チャットの更新日時を更新
        update_chat_stmt = update(Chat).where(Chat.id == request.chatId).values(
