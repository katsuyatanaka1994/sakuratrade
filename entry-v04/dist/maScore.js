 ...
 * @returns 傾きの方向
 */
function getSlopeDirection(slopePct, thresholdPct) {
    if (slopePct >= thresholdPct)
        return 'up';
    if (slopePct <= -thresholdPct)
        return 'down';
    return 'flat';
 ...
 * @returns スコア（0-100）
 */
function getSlopeScore(direction, maType) {
    const scoreTable = {
 ...
 * @returns MAスコア詳細
 */
export function calculateMAScore(indicators, options) {
    // 現在値の取得（null安全）
