 ...
 * MAの状態説明を生成
 */
function describeMAStatus(details) {
    const slopes = [];
    if (details.sma5SlopePct >= 0.5)
        slopes.push('5↗');
    else if (details.sma5SlopePct <= -0.5)
        slopes.push('5↘');
    else
        slopes.push('5→');
    if (details.sma20SlopePct >= 0.5)
        slopes.push('20↗');
    else if (details.sma20SlopePct <= -0.5)
        slopes.push('20↘');
    else
        slopes.push('20→');
    if (details.sma60SlopePct >= 0.5)
        slopes.push('60↗');
    else if (details.sma60SlopePct <= -0.5)
        slopes.push('60↘');
    else
        slopes.push('60→');
 ...
 * Candleの状態説明を生成
 */
function describeCandleStatus(details) {
    const patternName = PATTERN_NAMES[details.pattern] || details.pattern;
    const adjustments = [];
    if (details.adjustments.upperPenalty < 0)
        adjustments.push('上髭ペナ');
    if (details.adjustments.clvAdjustment > 0)
        adjustments.push('CLV+');
    if (details.adjustments.clvAdjustment < 0)
        adjustments.push('CLV-');
    if (details.adjustments.rangeAdjustment > 0)
        adjustments.push('拡大+');
    if (details.adjustments.rangeAdjustment < 0)
        adjustments.push('縮小-');
    if (details.adjustments.gapAdjustment < 0)
        adjustments.push('ギャップ失速');
 ...
 * 出来高の状態説明を生成
 */
function describeVolumeStatus(details) {
    const ratio = details.volumeRatio;
    if (ratio >= 2.0)
        return `${ratio.toFixed(1)}× 大幅増`;
    if (ratio >= 1.5)
        return `${ratio.toFixed(1)}× 増加`;
    if (ratio >= 1.2)
        return `${ratio.toFixed(1)}× やや増`;
    if (ratio >= 0.8)
        return `${ratio.toFixed(1)}× 標準`;
    return `${ratio.toFixed(1)}× 減少`;
}
/**
 * Entry用の詳細説明を生成
 */
export function generateEntryExplain(entry) {
    if (!entry.gatePassed) {
 ...
 * 押し目ロング総合用の説明を生成
 */
export function generateLongSetupExplain(result) {
 ...
 * チャット用の短縮メッセージを生成
 */
export function generateChatMessage(result) {
 ...
 * 失敗理由の詳細説明を生成
 */
export function generateFailureDetails(result) {
    const issues = [];
    // Pivot失敗
