 ...
 * @returns 有効な値または既定値
 */
export function nz(value: number | null | undefined, defaultValue: number = 0): number {
 ...
 * @returns 実体比率（0-1）
 */
export function bodyRatio(open: number, close: number, high: number, low: number): number {
 ...
 * @returns 上髭比率（0-1）
 */
export function upperShadowRatio(open: number, close: number, high: number, low: number): number {
  const range = Math.max(1e-9, high - low);
 ...
 * @returns 下髭比率（0-1）
 */
export function lowerShadowRatio(open: number, close: number, high: number, low: number): number {
  const range = Math.max(1e-9, high - low);
 ...
 * @returns CLV（0-1）
 */
export function calculateCLV(close: number, high: number, low: number): number {
 ...
 * @returns 陽線の場合true
 */
export function isBullish(open: number, close: number): boolean {
  return close > open;
}
 ...
 * @returns 陰線の場合true
 */
export function isBearish(open: number, close: number): boolean {
  return close < open;
}
 ...
 * @returns 近い場合true
 */
export function isNear(price: number, target: number, thresholdPct: number): boolean {
