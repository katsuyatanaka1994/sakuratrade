...
import type { Bar, Indicators, CandlePattern, CandleScoreDetails, Options } from './types.js';
import { 
  bodyRatio, 
  upperShadowRatio, 
  lowerShadowRatio, 
 ...
 * ブレイク系パターンの判定
 */
function isBreakoutPattern(
  bar: Bar, 
  indicators: Indicators, 
  bodyRatio: number, 
  upperRatio: number
): { pattern: CandlePattern; score: number } | null {
 ...
 * インサイド上放れパターンの判定
 */
function isInsideBreakoutPattern(
  bar: Bar, 
  indicators: Indicators
): { pattern: CandlePattern; score: number } | null {
  const prevHigh = nz(indicators.prevHigh);
  ...
  // インサイド条件（前日が前々日に内包）
  const prevInside = isInside(
    { high: prevHigh, low: prevLow },
    { high: prev2High, low: prev2Low }
  );
 ...
 * 20MAタッチ反転パターンの判定
 */
function isMA20TouchReversalPattern(
  bar: Bar, 
  indicators: Indicators, 
  lowerRatio: number,
  options: Options
 ...
 */
function isEngulfingAtMA20Pattern(
  bar: Bar, 
  indicators: Indicators,
  options: Options
): { pattern: CandlePattern; score: number } | null {
 ...
 * 続伸・小実体(HHHL)パターンの判定
 */
function isContinuationSmallBodyPattern(
  bar: Bar, 
  indicators: Indicators, 
  bodyRatio: number
): { pattern: CandlePattern; score: number } | null {
 ...
 * 汎用陽線パターンの判定
 */
function isGenericBullishPattern(
  bar: Bar, 
  bodyRatio: number
): { pattern: CandlePattern; score: number } | null {
  const bullish = isBullish(bar.open, bar.close);
...
function identifyPattern(
  ...
  indicators: Indicators, 
  options: Options
): { pattern: CandlePattern; baseScore: number } {
  ...
  if (breakout) return { pattern: breakout.pattern, baseScore: breakout.score };
  
  // 2. インサイド上放れ
  const insideBreakout = isInsideBreakoutPattern(bar, indicators);
  if (insideBreakout) return { pattern: insideBreakout.pattern, baseScore: insideBreakout.score };
  
  // 3. 20MAタッチ反転
  const ma20Touch = isMA20TouchReversalPattern(bar, indicators, lower, options);
  if (ma20Touch) return { pattern: ma20Touch.pattern, baseScore: ma20Touch.score };
  
  // 4. エンガルフィング@20MA
  const engulfing = isEngulfingAtMA20Pattern(bar, indicators, options);
  if (engulfing) return { pattern: engulfing.pattern, baseScore: engulfing.score };
  
  // 5. 続伸・小実体(HHHL)
  const continuation = isContinuationSmallBodyPattern(bar, indicators, body);
  if (continuation) return { pattern: continuation.pattern, baseScore: continuation.score };
  
  // 6. 汎用陽線
  const generic = isGenericBullishPattern(bar, body);
  if (generic) return { pattern: generic.pattern, baseScore: generic.score };
 ...
 * Candleスコアを計算
 */
export function calculateCandleScore(
  bar: Bar, 
  indicators: Indicators, 
  options: Options
): CandleScoreDetails {
  ...
  const adjustments = calculateAdjustments(
    pattern, upper, clv, rangeRatio, gapPct
  );
