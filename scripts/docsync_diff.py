#!/usr/bin/env python3
import json, sys, hashlib, yaml, os, pathlib
...
def main():
    ...
    plan = {
        ...
        "recommendation": (
            ...
            if not equal else "No action needed."
        ),
    }
    ...
    if not equal:
        print("::error title=DocSync drift detected::backend/app/openapi.yaml と docs/specs/openapi.yaml が不一致です。Actions → docsync-apply を実行してください。")
        print("DRIFT")
        sys.exit(1)
    else:
        print("EQUAL")
