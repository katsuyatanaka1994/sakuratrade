...
async def update_message(message_id: str, message_update: ChatMessageUpdate):
    ...
    if is_entry_settled(current_text) or is_entry_settled(new_text):
        ...
        raise HTTPException(status_code=409, detail="Cannot edit settled ENTRY message. Position is already closed.")
...
async def get_messages(chat_id: str, limit: Optional[int] = 100, offset: Optional[int] = 0):
    ...
    # チャットのメッセージを取得
    chat_messages = [msg for msg in messages_storage.values() if msg["chat_id"] == chat_id]
...
async def undo_message(message_id: str):
    ...
    # 時間制限チェック（30分以内のみ）
    created_at = datetime.fromisoformat(message["created_at"])  # aware (has tz)
    ...
    return {"message": "Message undone successfully", "message_id": message_id, "undone_at": now_iso_utc()}
...
async def generate_ai_reply(request: AIReplyRequest):
        ...
        try:
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "あなたは日本語で簡潔に答える投資アシスタントです。専門用語は短く補足してください。",
                    },
    ...
    if not ai_response:
        ai_response = (
            "AI応答: メッセージ「"
            f"{user_message.get('text', user_message.get('type', 'メッセージ'))}"
            "」に対する分析結果です。"
...
async def legacy_advice(request: AdviceRequest):
    ...
    return {"message": response_message, "status": "success", "chat_id": request.chat_id}
...
async def debug_messages():
    """デバッグ用: 全メッセージを表示"""
    return {"messages": list(messages_storage.values()), "message_count": len(messages_storage)}


@app.get("/debug/chats")
async def debug_chats():
    """デバッグ用: 全チャットを表示"""
    return {"chats": list(chats_storage.values()), "chat_count": len(chats_storage)}
...
async def submit_journal_entry(trade_snapshot: TradeSnapshotModel):
    ...
    try:
        ...
        journal_entry = JournalEntryModel(
            ...
            exit_time=trade_snapshot.exitTime or now_iso_utc(),
