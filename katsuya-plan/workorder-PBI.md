...
- **直近の課題**
  - `workflow_dispatch` ルートでは `needs.sync.outputs.should_run` が `true` になっても jobs が生成されず、`workorder_ready` へ outputs が渡らない（GitHub UI 上は run failure でログ未生成）。原因は default branch(main) に workflow_dispatch を持つ plan-sync が存在しないこと。main へブリッジ workflow を配置しない限り branch 版 plan-sync を dispatch 経由で呼べない。
  - `pull_request_target(labeled)` ルートは main 上の workflow を実行するため、branch で加えた gate/`workflow_call` 変更が反映されず `has_plan_diff == true` でも `workorder_ready` まで到達できない。Case1 を取得するには dispatch ブリッジを main へ追加→workflow_call で branch 版を起動する必要がある。
...
- **取組状況**
  ...
  - plan-sync に `workflow_call` イベントを追加し、main 向けには `plan-sync-dispatch-bridge.yml` を新設（`workflow_dispatch` → branch 版 plan-sync を `uses` 呼び出し）。レポジトリルールで main 直 push が禁止のため、ブリッジ workflow は PR 経由で main に取り込む必要がある。
  - Case1 用に `doc_sync_plan.json` へ仮の `test_flows` エントリを追加して AUTO 差分を強制し、branch 版 plan-sync で `has_plan_diff=true` になるよう準備した（commit `bda4cc4`）。
