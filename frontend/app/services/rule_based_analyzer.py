...
class RuleBasedAnalyzer:
    ...
    def analyze_entry_v04(self, bar_data: Dict[str, Any], indicators_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Entry v0.4 分析を実行"""
        try:
            # Node.js経由でentry分析実行
            analysis_input = {
    ...
    def pivot_result_to_indicators(self, pivot_result: Dict[str, Any]) -> List[IndicatorItem]:
        ...
        indicators.append(IndicatorItem(
            ...
            comment=f"移動平均線の傾き評価。20MA: {slope20_pct:+.2f}%, 60MA: {slope60_pct:+.2f}%。スコア{slope_score}点。",
            source="rule_based",
            confidence=0.85
        ))
    ...
    def entry_result_to_indicators(self, entry_result: Dict[str, Any]) -> List[IndicatorItem]:
        ...
        indicators.append(IndicatorItem(
            ...
            comment=f"Entry v0.4による総合判定: {label}。最終スコア{final_score}点、ゲート通過: {'○' if gate_passed else '×'}",
            source="rule_based",
            confidence=0.90
        ))
    ...
    def _get_fallback_entry_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'label': '見送り',
            'gatePassed': False,
            'scores': {'MA': 50, 'Candle': 50, 'Volume': 50}
        }