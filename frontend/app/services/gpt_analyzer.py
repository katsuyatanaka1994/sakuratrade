...
class GPTAnalyzer:
    ...
    def analyze_chart_image(
        self, image_base64: str, symbol_context: str = None, analysis_context: str = None
    ) -> Dict[str, Any]:
...
"""
        ...
        user_prompt = (
            ...
            f"{analysis_context or 'テクニカル指標'}を上記JSON形式で抽出してください。"
        )
    ...
    def _parse_natural_text(self, content: str) -> Dict[str, Any]:
        ...
        if any(word in content for word in ["上昇", "上向き", "強気"]):
            result["trend"] = {"value": "上昇トレンド", "evaluation": "強気", "comment": "上昇トレンドが継続中"}
        elif any(word in content for word in ["下降", "下向き", "弱気"]):
            result["trend"] = {"value": "下降トレンド", "evaluation": "弱気", "comment": "下降トレンドが継続中"}
    ...
    def _get_fallback_gpt_result(self) -> Dict[str, Any]:
        """GPT分析エラー時のフォールバック結果"""
        return {
            "rsi": {"value": 50, "evaluation": "中立", "comment": "RSI分析データを取得できませんでした"},
            "trend": {"value": "不明", "evaluation": "中立", "comment": "トレンド分析データを取得できませんでした"},
            "volume": {"value": "不明", "evaluation": "中立", "comment": "出来高分析データを取得できませんでした"},
        }
